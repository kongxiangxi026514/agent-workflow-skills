...
模型路由由 `config/model-routing.md` 统一说明;OpenCode 的 build/review 角色使用独立原生 agent 文件。安装/卸载全部走脚本,**不需要手动移动或合并任何文件**。
...
| `opencode/agents/{build,review}.md` | 配置 | OpenCode 原生 build/review agent,不写主配置 | 安装后自动发现 |

## 一键安装(脚本,零手动拷贝)

安装脚本会自动放置全部 5 个 skill、强制脊柱和 OpenCode 原生 agent。文本按 UTF-8 处理,重复安装幂等。OpenCode 的 `opencode.json` / `opencode.jsonc` 只做 UTF-8 与语法预检,**始终逐字节保持不变**;两者同时存在或任一文件损坏时,在任何写入前失败。
...
- **单一真源** = `config/model-routing.md`(角色 → 模型)。OpenCode 独立 agent 文件保留同一 Terra / Sol / GLM 角色策略。
...
- **换模型 = 改单一真源并同步镜像**:同步 `rules/workflow-gate.mdc`、`skills/parallel-dispatch/SKILL.md` 与 `opencode/agents/` 的角色说明;不向用户主配置注入 model key。
...
- **OpenCode(全局,脚本全自动)**:5 个 skill 写入 `~/.config/opencode/skills/`,脊柱幂等注入 `AGENTS.md`,build/review 写入 `agents/*.md`;主配置 `.json` / `.jsonc` 不改、不新建。完成后重启 OpenCode。
