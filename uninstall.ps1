...
function Test-CursorOwnership {
    ...
    if ($globalCandidate) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw 'Cursor global skill ownership validation failed. Nothing was uninstalled.'
        }
    }
    if ($projectCandidate) {
        if (-not $Project -or -not (Test-Path -LiteralPath $state)) {
            throw 'Cursor project artifacts require a valid project install-state.json. Nothing was uninstalled.'
        }
        & $python (Join-Path $RepoRoot 'tools\verify_cursor_ownership.py') `
            '--state' $state '--rules' $rules '--skills' $skills '--bundle' $bundle | Out-Null
        if ($LASTEXITCODE -ne 0) {
            throw 'Cursor project ownership validation failed. Nothing was uninstalled.'
        }
    }
}
